...
export default function SearchableSelect({
  ...
  const triggerClass = [
    'w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white flex items-center justify-between gap-2 text-start',
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute z-50 start-0 end-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl overflow-hidden">
          <div className="p-2 border-b border-gray-100">
            <input
              ...
              className="w-full px-2 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-400 bg-gray-50"
            />
          </div>
          <ul ref={listRef} className="max-h-52 overflow-y-auto py-1">
            ...
            {filtered.length === 0 && (
              <li className="px-3 py-2 text-sm text-gray-400 text-center">{t('app.noItems')}</li>
            )}
            {filtered.map((opt, i) => (
              <li
                key={opt.value}
