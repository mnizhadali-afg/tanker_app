    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-gray-200">
          ...
          <button
            onClick={onClose}
            className="ms-3 text-gray-400 hover:text-gray-600 w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100 text-xl leading-none cursor-pointer shrink-0"
          >
            ×
          </button>
        </div>
            ...
            {fields.map((f, i) => (
              <div key={i} className={f.wide ? 'col-span-2' : ''}>
                ...
                <div className="text-sm font-medium text-gray-800 break-words">
                  {f.value !== undefined && f.value !== null && f.value !== '' ? f.value : <span className="text-gray-400 font-normal">—</span>}
                </div>
              </div>
            ))}
          </div>
