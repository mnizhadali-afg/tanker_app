...
export default function CostBasedTemplate({ invoice }: Props) {
  const { t, i18n } = useTranslation()
  const locale = i18n.language

  // Customer cost columns only (exclude producer, per-ton, identity, calculated)
  const costCols = TANKER_COLUMNS.filter(
    (c) =>
      ...
      !c.isCalculated &&
      c.group !== 'per-ton' &&
      !['productWeight', 'billWeight', 'exchangeRate', 'commodityPercentDebt'].includes(c.key),
  )

  // Zero-suppress: hide columns where ALL rows have value 0
        ...
        <tbody>
          {invoice.tankers.map((tanker, i) => (
            <tr key={tanker.id}>
              ...
              <td>{formatNumber(tanker.customerDebtAfn as number, locale)}</td>
