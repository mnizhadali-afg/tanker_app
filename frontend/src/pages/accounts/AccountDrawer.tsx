      ...
      >
        ...
        {account && (
          <div className="px-5 py-4 border-t border-gray-200 flex gap-3">
            <button
              onClick={() => { onClose(); navigate(`/accounts/${account.id}/edit`) }}
...
function BalanceRow({ label, value, isDebt, currency }: { label: string; value: string; isDebt: boolean; currency?: string }) {
  return (
    <div className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-gray-50">
      <span className="text-sm text-gray-600">{label}</span>
      <span className={`text-sm font-semibold ${isDebt ? 'text-red-600' : 'text-green-600'}`}>
        {value} {currency && <span className="font-normal text-xs text-gray-500">{currency}</span>}
      </span>
    </div>
  )
}
