      ...
      <div className='flex gap-1 bg-gray-100 rounded-lg p-1 w-fit'>
        {TABS.map((tb) => (
          <button
            key={tb}
            onClick={() => setTab(tb)}
            className={`hover:cursor-pointer text-sm px-4 py-1.5 rounded-md transition-colors ${tab === tb ? 'bg-white shadow-sm font-medium text-gray-900' : 'text-gray-600 hover:text-gray-900'}`}
          >
            {t(`reports.${tb}`)}
          </button>
        ))}
