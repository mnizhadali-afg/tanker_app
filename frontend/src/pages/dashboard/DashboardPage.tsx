...
function Card({
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden ${className}`}
    >
      {children}
    </div>
  );
...
function EmptyRow({ label }: { label: string }) {
  return (
    <p className='px-4 py-6 text-xs text-gray-400 dark:text-slate-500 text-center'>
      {label}
    </p>
  );
}
...
export default function DashboardPage() {
      ...
      <div className='space-y-4'>
        <div className='grid grid-cols-4 gap-4'>
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className='h-24 animate-pulse bg-gray-100 dark:bg-slate-700 rounded-xl'
            />
          ))}
        </div>
  ...
  const maxCustomerAfn = Math.max(
    ...data.topCustomers.map((c) => c.balanceAfn),
    1,
  );
  const maxProducerAfn = Math.max(
    ...data.topProducers.map((p) => p.balanceAfn),
    1,
  );
    ...
    <div className='space-y-4'>
      ...
      <div className='grid grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <Card>
          <div className='p-4 border-s-4 border-rose-400 dark:border-rose-500'>
            <p className='text-xs text-gray-500 dark:text-slate-400 mb-1'>
              {t('accounts.types.customer')} — {t('currency.afn')}
            </p>
            <p
              className={`text-2xl font-bold tabular-nums ${data.totalBalanceAfn > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-gray-700 dark:text-slate-300'}`}
            >
        ...
        <Card>
          <div className='p-4 border-s-4 border-rose-300 dark:border-rose-600'>
            <p className='text-xs text-gray-500 dark:text-slate-400 mb-1'>
              {t('accounts.types.customer')} — {t('currency.usd')}
            </p>
            <p
              className={`text-2xl font-bold tabular-nums ${data.totalBalanceUsd > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-gray-700 dark:text-slate-300'}`}
            >
        ...
        <Card>
          <div className='p-4 border-s-4 border-amber-400 dark:border-amber-500'>
            <p className='text-xs text-gray-500 dark:text-slate-400 mb-1'>
              {t('accounts.types.producer')} — {t('currency.afn')}
            </p>
            <p
              className={`text-2xl font-bold tabular-nums ${data.totalProducerPayableAfn > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-slate-300'}`}
            >
              {formatNumber(data.totalProducerPayableAfn, locale)}
            </p>
            <p className='text-xs text-gray-400 dark:text-slate-500 mt-1'>
              {t('dashboard.producerPayable')}
            </p>
          </div>
        </Card>
        ...
        <Card>
          <div className='p-4 border-s-4 border-amber-300 dark:border-amber-600'>
            <p className='text-xs text-gray-500 dark:text-slate-400 mb-1'>
              {t('accounts.types.producer')} — {t('currency.usd')}
            </p>
            <p
              className={`text-2xl font-bold tabular-nums ${data.totalProducerPayableUsd > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-slate-300'}`}
            >
      ...
      <Card>
        <div className='px-5 py-3 flex items-center gap-2 flex-wrap'>
          <span className='text-xs font-semibold text-gray-500 dark:text-slate-400 me-2'>
            {t('nav.invoices')}:
          </span>
          ...
          >
            ...
            <span className='text-xs font-bold text-yellow-800 dark:text-yellow-300 tabular-nums'>
              {fmtCount(data.invoiceCounts.draft)}
            </span>
          </button>
          ...
          >
            ...
            <span className='text-xs text-green-700 dark:text-green-400'>
              {t('invoices.statuses.final')}
            </span>
            <span className='text-xs font-bold text-green-800 dark:text-green-300 tabular-nums'>
              {fmtCount(data.invoiceCounts.final)}
            </span>
          </button>
          ...
          >
            ...
            <span className='text-xs text-red-700 dark:text-red-400'>
              {t('invoices.statuses.canceled')}
            </span>
            <span className='text-xs font-bold text-red-800 dark:text-red-300 tabular-nums'>
              {fmtCount(data.invoiceCounts.canceled)}
            </span>
          </button>
        </div>
      </Card>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
        <Card>
          <CardHeader
            ...
            action={<ViewAllBtn onClick={() => navigate('/reports')} />}
          />
        ...
        <Card>
          <CardHeader
            title={t('dashboard.topProducers')}
            action={<ViewAllBtn onClick={() => navigate('/reports')} />}
          />
        ...
        <div className='lg:col-span-3'>
          <Card className='h-full'>
            <CardHeader
              ...
              action={<ViewAllBtn onClick={() => navigate('/invoices')} />}
            />
            {data.recentInvoices.length === 0 ? (
              ...
              data.recentInvoices.map((inv) => (
                ...
                >
                  ...
                  <StatusBadge
                    status={inv.status}
                    label={t(`invoices.statuses.${inv.status}`)}
                  />
                </button>
              ))
            )}
          ...
          <CardHeader
            ...
            action={
              data.draftInvoices.length > 0 ? (
                <ViewAllBtn onClick={() => navigate('/invoices')} />
              ) : undefined
            }
          />
          {data.draftInvoices.length === 0 ? (
            ...
            data.draftInvoices.map((inv) => (
              ...
              >
                ...
                <p className='text-xs text-gray-400 dark:text-slate-500 truncate w-full text-start'>
                  {inv.customer.name}
                </p>
              </button>
            ))
      ...
      {data.recentPayments.length > 0 && (
        <Card>
          <CardHeader
            ...
            action={<ViewAllBtn onClick={() => navigate('/payments')} />}
          />
