...
export default function PortFormPage({ formId, onSuccess, onCancel }: Props = {}) {
  ...
  const formBody = (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{t('ports.name')}</label>
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className={inputClass} />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{t('ports.producer')}</label>
        <SearchableSelect
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-slate-100">{isEdit ? t('app.edit') : t('ports.new')}</h1>
      </div>
