...
export default function PortsListPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    setDeleteError('');
    try {
      await Promise.all([...selectedIds].map((id) => api.delete(`/ports/${id}`)));
      setPorts((prev) => prev.filter((p) => !selectedIds.has(p.id)));
  ...
  const selectedNames = ports.filter((p) => selectedIds.has(p.id)).map((p) => p.name);
  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <h1 className='text-xl font-bold text-gray-900 dark:text-slate-100'>{t('ports.title')}</h1>
        <button
